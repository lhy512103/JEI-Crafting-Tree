...
public final class Ae2BatchCraftBootstrap {
    ...
    public static BatchCraftResultPayload execute(ServerPlayer player, BatchCraftRequestPayload request) {
        if (!(player.containerMenu instanceof CraftingTermMenu menu)
                ...
                || !ItemStack.isSameItemSameTags(resultSlot.getItem(), request.expectedOutput())
        ...
        for (int batch = 0; batch < MAX_STACK_BATCHES && System.nanoTime() < deadline; batch++) {
            ...
            if (currentOutput.isEmpty()
                    || !ItemStack.isSameItemSameTags(currentOutput, request.expectedOutput())) break;

            resultSlot.doClick(InventoryAction.CRAFT_STACK, player);
            ItemStack produced = menu.getCarried();
            if (produced.isEmpty()
                    || !ItemStack.isSameItemSameTags(produced, request.expectedOutput())) break;

            int producedAmount = produced.getCount();
            crafted += producedAmount;
            AEItemKey key = AEItemKey.of(produced);
            long inserted = 0L;
            // The Forge 1.20.1 AE2 menu exposes the carried result but not the newer
            // link/energy accessors. Leave the result in the player's inventory here.
            if (key != null) {
                player.getInventory().add(produced.copy());
                inserted = producedAmount;
            }
